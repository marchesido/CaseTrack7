...
export class GoogleCalendarController {
  ...
  @ApiOperation({
    summary:
      'Obter URL para autenticação e consentimento OAuth2 do Google Calendar',
  })
  @ApiResponse({
    status: 200,
    description: 'URL de consentimento gerada com sucesso.',
  })
  getAuthUrl() {
  ...
  @ApiOperation({
    summary: 'Conectar ao Google Calendar via código de autorização OAuth2',
  })
  @ApiResponse({
    status: 200,
  ...
  @Get('google/callback')
  @ApiOperation({
    summary: 'Endpoint de callback HTTP para redirecionamento OAuth2 do Google',
  })
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Consultar status da conexão global do Google Calendar',
  })
  ...
  @ApiOperation({
    summary:
      'Sincronizar produção e cronograma de diárias com o Google Calendar',
  })
  @ApiResponse({
    status: 200,
    description: 'Produção sincronizada no Google Calendar com sucesso.',
  })
  @ApiResponse({ status: 404, description: 'Produção não encontrada.' })
  @ApiResponse({
    status: 424,
    description:
      'Reconexão com o Google necessária (GOOGLE_RECONNECT_REQUIRED).',
  })
